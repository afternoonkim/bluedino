...
export const companyAnalysisMarkets: CompanyAnalysisMarketConfig[] = [
  ...
  },
  {
    ...
    intro:
      "해외기업 분석은 기업의 성장성뿐 아니라 환율, 글로벌 경쟁 구도, 금리, 규제, 산업 사이클까지 같은 기준으로 놓고 확인해야 합니다. BlueDino는 미국주식 주요 1,000개 기업을 처음 보는 사용자도 핵심 사업과 투자 전 확인할 부분을 빠르게 이해할 수 있도록 정리합니다.",
...
function getValuationLens(sector: string) {
  if (/바이오|제약|신약|임상/i.test(sector)) return "단순 PER보다 현금 보유액, 파이프라인 가치, 기술이전 가능성, 승인 확률을 같은 기준으로 놓고 확인해야 합니다";
  if (/리츠|부동산/i.test(sector)) return "주가순자산비율, 배당수익률, 임대율, 차입금 만기 구조가 먼저 볼 기준입니다";
  ...
  if (/반도체|2차전지|조선|화학|정유|철강/i.test(sector)) return "업황 저점과 고점이 반복되므로 평균 이익 기준의 밸류에이션과 사이클 위치를 같은 기준으로 놓고 확인해야 합니다";
...
function buildArticle(seed: CompanySeed, market: CompanyAnalysisMarket): CompanyAnalysisArticle {
  ...
  const metaDescription = `${seed.companyNameKo}(${seed.ticker}) 기업분석입니다. ${seed.sector} 사업 구조, 성장 포인트, 실적 확인 지표, 주가 리스크와 투자 전 체크포인트를 초보 투자자도 이해하기 쉽게 비교할 수 있게 구성했습니다.`;
    ...
    sections: [
      (() => {
        ...
        const baseBody = [
          ...
          `특히 ${theme} 흐름 안에서 ${seed.companyNameKo}가 단순 테마에 묶여 움직이는 기업인지, 실제 매출과 이익을 만드는 기업인지 구분하는 것이 먼저 볼 기준입니다. 같은 업종이라도 고객사 구성, 가격 결정력, 기술 장벽, 원가 구조에 따라 실적 민감도는 크게 달라질 수 있습니다.`,
          `실제 투자 판단에서는 "유명한 기업인가"보다 "실적이 반복적으로 쌓일 수 있는 구조인가"가 더 먼저 볼 기준입니다. 매출원이 특정 고객이나 특정 제품에 과도하게 집중되어 있다면 성장성이 커 보여도 변동성이 커질 수 있습니다.`,
        ];
        return {
      ...
      (() => {
        ...
        return {
          ...
          body: [
            opener2,
            `산업 분석에서 중요한 것은 뉴스의 크기가 아니라 실적 연결 가능성입니다. 투자자는 해당 산업의 수요 증가가 실제 주문, 가격, 마진, 현금흐름으로 이어지고 있는지 단계별로 확인해야 합니다.`,
      ...
      })(),
      {
        ...
        body: [
          ...
          `분기 실적은 일시적인 변수에 흔들릴 수 있으므로 최소 3~4개 분기의 흐름을 함께 보는 흐름이 현실적입니다. 실적 발표 후에는 회사의 가이던스와 시장 기대치가 얼마나 차이 나는지도 확인해야 합니다.`,
        ],
      },
      {
      ...
      })(),
      {
        ...
        body: [
          ...
          `특히 ${seed.sector} 기업은 산업 변화가 빠르게 나타날 수 있으므로 경쟁사의 신규 투자, 대체 기술 등장, 고객사의 내재화 가능성까지 같은 기준으로 놓고 확인해야 합니다.`,
        ],
      },
      {
        ...
        body: [
          ...
          `비교할 때는 단순히 시가총액 순위만 보지 말고 매출 성장률, 영업이익률, 순이익률, 잉여현금흐름, 부채 부담을 같은 기준으로 정리하는 방식이 적합합니다. 이 과정에서 ${seed.companyNameKo}가 성장주로 평가받는지, 가치주로 평가받는지, 경기민감주로 움직이는지 더 선명하게 볼 수 있습니다.`,
      ...
      },
      {
        ...
        body: [
          `${seed.companyNameKo}의 상승 시나리오는 ${theme} 흐름이 실제 실적 개선으로 확인되고, 시장이 이익 성장의 지속성을 다시 평가할 때 만들어질 수 있습니다. 단기 뉴스보다 수주, 판매량, 가격, 마진, 현금흐름이 동시에 개선되는지가 먼저 볼 기준입니다.`,
      ...
      },
      {
        title: `리스크 요인과 대응 관점`,
        body: [
          `${seed.companyNameKo}의 주요 리스크는 ${riskTheme}입니다. 리스크를 단순히 부정적인 요소로만 볼 것이 아니라, 이미 주가에 반영된 리스크인지 아직 반영되지 않은 리스크인지 나눠서 보는 것이 먼저 볼 기준입니다.`,
      ...
      },
      {
        ...
        body: [
          `${seed.companyNameKo}를 처음 보는 투자자라면 바로 매수 여부를 결정하기보다 관심 기업 목록에 넣고 실적 발표, 산업 뉴스, 동종 기업 흐름을 함께 추적하는 방식이 적합합니다.`,
    ...
    faq: [
      {
        ...
        answer:
          `${seed.companyNameKo} 주가 전망은 단기 뉴스보다 매출 성장률, 영업이익률, 산업 수요, 경쟁 구도, 밸류에이션 부담을 같은 기준으로 놓고 확인해야 합니다. 특히 기대치가 높은 종목은 실적이 좋아도 시장 예상보다 낮으면 주가가 흔들릴 수 있습니다.`,
      },
      {
        question: `${seed.companyNameKo}는 장기투자 관점에서 어떻게 봐야 하나요?`,
        answer:
          `장기투자 관점에서는 ${seed.companyNameKo}가 속한 ${seed.sector} 산업의 성장성이 실제 실적으로 이어지는지 확인해야 합니다. 단순히 유명한 기업이라는 이유보다 현금흐름, 이익률, 경쟁 우위가 유지되는지가 더 먼저 볼 기준입니다.`,
      },
      {
        question: `${seed.companyNameKo} 투자 전 어떤 리스크를 확인해야 하나요?`,
        answer:
          `${seed.companyNameKo} 투자 전에는 ${riskTheme}을 먼저 확인해야 합니다. 여기에 금리, 환율, 규제, 경쟁사의 전략 변화까지 함께 보면 주가 변동의 이유를 더 잘 이해할 수 있습니다.`,
      },
    ],
    tags: [
